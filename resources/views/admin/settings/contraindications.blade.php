...
@section('content')
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Add New Contraindications</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('contraindicationsSelection.store') }}" method="post">
                        @csrf
                        <div class="form-group">
                            <label for="recipient-name" class="col-form-label">Contraindications Name:</label>
    ...
    <div class="row px-5">
        <div class="col-12">
            <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                    @forelse ($contraindications as $contraindication)
                        <tr>
                            <td>{{ $contraindication->id }}</td>
                            <td>{{ $contraindication->name }}</td>
                            {{-- <td>{{ $category->category->name ?? 'Category deleted' }}</td> --}}
                            <td>
                                <!-- Button to trigger delete confirmation modal -->
                                <button class="btn btn-danger" data-toggle="modal"
                                    data-target="#deleteModal{{ $contraindication->id }}">Delete</button>

                                <!-- Delete Confirmation Modal -->
                                <div class="modal fade" id="deleteModal{{ $contraindication->id }}" tabindex="-1" role="dialog"
                                    ...
                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <form action="{{ route('contraindicationsSelection.destroy', $contraindication->id) }}"
                                                    method="post">
                                                    @method('DELETE')
                                                    @csrf
    ...
    {{ $contraindications->withQueryString()->links() }}
@endsection
