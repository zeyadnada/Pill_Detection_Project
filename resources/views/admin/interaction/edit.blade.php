...
@section('content')
    <div class="row">
        <div class="col-12">
            <form action="{{ route('pillsInteraction.update', $interaction->id) }}" method="post">
                @csrf
                @method('PUT')
                <div class="form-row">
                    <div class="col-5">
                        <label for="pill_1_id">Pill #1</label>
                        <select class="form-control select2 select2-hidden-accessible" name="pill_1_id" style="width: 100%;"
                            data-select2-id="1" tabindex="-1" aria-hidden="true">
                            @foreach ($pills as $pill)
                                <option {{ $pill->id == $interaction->pill_1_id ? 'selected' : '' }}
                                    value="{{ $pill->id }}">{{ $pill->name }}</option>
                            @endforeach
                        </select>
                        @error('pill_1_id')
                    ...
                    <div class="col-5">
                        ...
                        <select class="form-control select2 select2-hidden-accessible" name="pill_2_id" style="width: 100%;"
                            data-select2-id="2" tabindex="-1" aria-hidden="true">
                            @foreach ($pills as $pill)
                                <option {{ $pill->id == $interaction->pill_2_id ? 'selected' : '' }}
                                    value="{{ $pill->id }}">{{ $pill->name }}</option>
                            @endforeach
                        </select>
                        @error('pill_2_id')
                    ...
                    <div class="col-2">
                        ...
                        <select class="form-control select2 select2-hidden-accessible" name="interaction_type"
                            ...
                            <option {{ $interaction->interaction_type == 'severe' ? 'selected' : '' }} value="severe">
                                Severe
                            </option>
                            <option {{ $interaction->interaction_type == 'moderate' ? 'selected' : '' }} value="moderate">
                                Moderate</option>
                            <option {{ $interaction->interaction_type == 'conditional' ? 'selected' : '' }}
                                value="conditional">Potential/Conditional</option>
                            <option {{ $interaction->interaction_type == 'unknown' ? 'selected' : '' }} value="unknown">
                                Unknown</option>
                        </select>
                ...
                <div class="form-row">
                    <div class="col-12">
                        ...
                        <textarea class="form-control @error('interaction_description') is-invalid @enderror form-control custom-file"
                            ...
                            rows="5">{{ $interaction->interaction_description }}</textarea>
                ...
                <div class="form-row">
                    <div class="col-12">
                        ...
                        <textarea class="form-control @error('guides') is-invalid @enderror form-control custom-file" id="guides"
                            name="guides" placeholder="Enter Pill Guides" rows="4">{{ $interaction->guides }}</textarea>
                        @error('guides')
                            <div class="error alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
